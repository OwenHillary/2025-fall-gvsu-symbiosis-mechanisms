...
- + controls with no symbionts @ death rates of 0.0, 0.5, 0.75

Questions:
- Where do we get extinctions?
- Where do we get sustained co-evolution?
...
experiment_slug <- "2025-09-15-stress-parasites"
...
summary_data <- summary_data %>%
  ...
  mutate(
    treatment = case_when(
      START_MOI == "0" ~ paste0("NoSyms_SF=", STRESS_FREQUENCY, "_BDC=", BASE_DEATH_CHANCE), # nolint: line_length_linter.
      START_MOI == "1" ~ paste0(
        "SF=", STRESS_FREQUENCY, "_PDC=", PARASITE_DEATH_CHANCE, "_PO=", PARASITE_NUM_OFFSPRING_ON_STRESS_INTERACTION # nolint: line_length_linter.
      )
    )
  ) %>%
  mutate(
...
### Using tasks completed by parents

```{r}
# Pivot task data long:
# 1) Select only relevant columns
final_parent_task_data <- summary_data %>%
  select(
    SEED,
    START_MOI,
    BASE_DEATH_CHANCE,
...
final_parent_task_done <- final_parent_task_data %>%
  group_by(
    START_MOI,
    BASE_DEATH_CHANCE,
    STRESS_FREQUENCY,
...
```{r}
final_parent_task_done_plt <- final_parent_task_done %>%
  ggplot(
...
ggsave(
  file = paste(plot_dir, "final_parent_task_done_plt.pdf", sep = "/"),
  plot = final_parent_task_done_plt,
  width = 20,
  height = 20
)
```
...
# Pivot task data long:
# 1) Select only relevant columns
over_time_parent_task_data <- time_data %>%
  select(
    SEED,
    START_MOI,
    BASE_DEATH_CHANCE,
...
parent_tasks_all_time_by_task <- over_time_parent_task_data %>%
  group_by(
    SEED,
    treatment,
    task,
...
```{r}
parent_tasks_all_time_by_task_plt <- parent_tasks_all_time_by_task %>%
  ggplot(
...
ggsave(
  file = paste(plot_dir, "parent_tasks_all_time_by_task_plt.pdf", sep = "/"),
  plot = parent_tasks_all_time_by_task_plt,
  width = 20,
  height = 20
)
```
...
```{r}
total_different_parent_tasks_all_time <- over_time_parent_task_data %>%
  group_by(
    SEED,
    treatment,
    task,
...
```{r}
total_different_parent_tasks_all_time_plt <- total_different_parent_tasks_all_time %>%
  ggplot(
...
ggsave(
  file = paste(plot_dir, "total_different_parent_tasks_all_time_plt.pdf", sep = "/"),
  plot = total_different_parent_tasks_all_time_plt,
  width = 25,
  height = 20
)
```
...
```{r}
avg_tasks_table <- total_different_parent_tasks_all_time %>%
  group_by(treatment) %>%
...
save_kable(
  ...
  paste(
    plot_dir,
    "avg_diff_num_host_parent_tasks.pdf",
    sep = "/"
  )
)
```
...
save_kable(
  ...
  paste(
    plot_dir,
    "avg_diff_num_sym_parent_tasks.pdf",
    sep = "/"
  )
)
```