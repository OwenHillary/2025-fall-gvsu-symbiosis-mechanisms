...
experiment_slug <- "2025-09-19-stress-harshness"
...
summary_data <- summary_data %>%
  mutate(
    SEED = as.factor(SEED),
    STRESS_TYPE = as.factor(STRESS_TYPE),
  ...
  mutate(
    treatment = case_when(
      STRESS_TYPE == "mutualist" ~ paste0(
        "Mutualist_SF=", STRESS_FREQUENCY, "_BDC=", BASE_DEATH_CHANCE
      ),
      STRESS_TYPE == "parasite" ~ paste0(
        "Parasite_SF=", STRESS_FREQUENCY, "_PDC=", PARASITE_DEATH_CHANCE
      )
    ),
    stress_death_chance = case_when(
      STRESS_TYPE == "mutualist" ~ BASE_DEATH_CHANCE,
      STRESS_TYPE == "parasite" ~ PARASITE_DEATH_CHANCE
    )
  ) %>%
  mutate(
    treatment = as.factor(treatment),
    stress_death_chance = as.factor(stress_death_chance)
  )
```
...
```{r}
host_with_parasite_parent_tasks_all_time_by_task_plt <- host_parent_tasks_all_time_by_task %>%
  filter(STRESS_TYPE == "parasite") %>%
  ggplot(
    aes(x = treatment, color = treatment, fill = treatment, y = parent_tasks_completed_ever)
  ) +
  geom_col(alpha = 0.5) +
  scale_x_discrete(
    breaks = c(
      "Parasite_SF=25_PDC=0.25",
      "Parasite_SF=25_PDC=0.5",
      "Parasite_SF=25_PDC=0.75",
      "Parasite_SF=50_PDC=0.25",
      "Parasite_SF=50_PDC=0.5",
      "Parasite_SF=50_PDC=0.75",
      "Parasite_SF=100_PDC=0.25",
      "Parasite_SF=100_PDC=0.5",
      "Parasite_SF=100_PDC=0.75"
    ),
    limits = c(
      "Parasite_SF=25_PDC=0.25",
      "Parasite_SF=25_PDC=0.5",
...
ggsave(
  file = paste(plot_dir, "host_with_parasite_parent_tasks_all_time_by_task_plt.pdf", sep = "/"),
  plot = host_with_parasite_parent_tasks_all_time_by_task_plt,
  width = 10,
  height = 10
)
```
...
host_total_different_parent_tasks_all_time_plt <- host_total_different_parent_tasks_all_time %>%
  ggplot(
    aes(x = STRESS_TYPE, fill = STRESS_TYPE, y = num_parent_tasks_completed_ever)
  ) +
  geom_point(
    mapping=aes(color = STRESS_TYPE),
