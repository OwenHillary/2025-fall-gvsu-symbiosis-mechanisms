...
experiment_slug <- "2025-09-25-health-parasites"
...
summary_data <- summary_data %>%
  ...
  mutate(
    treatment = paste0(
      "IC=", HEALTH_INTERACTION_CHANCE, "_PBC=", PARASITE_BASE_CYCLE_PROP, "_SM=", PARASITE_CYCLE_STEAL_MULTIPLIER # nolint: line_length_linter.
    )
  ) %>%
  mutate(
...
host_total_different_parent_tasks_all_time_plt <- host_total_different_parent_tasks_all_time %>%
  ggplot(
    aes(x = HEALTH_INTERACTION_CHANCE, fill = HEALTH_INTERACTION_CHANCE, y = num_parent_tasks_completed_ever)
  ) +
  geom_point(
    mapping=aes(color = HEALTH_INTERACTION_CHANCE),
  ...
  facet_wrap(
    PARASITE_BASE_CYCLE_PROP~PARASITE_CYCLE_STEAL_MULTIPLIER,
    labeller = label_both
  ) +
  theme(
    legend.position = "none",
...
```{r}
host_total_different_parent_tasks_all_time_plt <- host_total_different_parent_tasks_all_time %>%
  filter(HEALTH_INTERACTION_CHANCE == "1") %>%
  ggplot(
    aes(x = PARASITE_BASE_CYCLE_PROP, fill = PARASITE_BASE_CYCLE_PROP, y = num_parent_tasks_completed_ever)
  ) +
  geom_point(
    mapping=aes(color = PARASITE_BASE_CYCLE_PROP),
    position = position_jitter(width = .15),
    size = .5,
    alpha = 0.8
  ) +
...
ggsave(
  file = paste(plot_dir, "host_HIC=1.0_total_different_parent_tasks_all_time_plt.pdf", sep = "/"),
  plot = host_total_different_parent_tasks_all_time_plt,
  width = 30,
  height = 15
)
