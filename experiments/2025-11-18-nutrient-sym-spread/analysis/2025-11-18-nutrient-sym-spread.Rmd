...
connected %>%
  ggplot( aes(x=update, y=n, group=name, color=name)) +
      geom_line() +
      scale_color_viridis(discrete = TRUE) +
      ggtitle("Simiularty between")+
      theme_ipsum() +
...
```{r}
connected_plt <- connected %>%
  ggplot(
    aes(x = connected, color = treatment, fill = treatment, y = update)
  ) +
...
ggsave(
  file = paste(plot_dir, "Connected.pdf", sep = "/"),
  plot = connected_plt,
  width = 20,
  height = 20
)
